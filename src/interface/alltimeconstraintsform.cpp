...
void AllTimeConstraintsForm::modifyConstraint()
{
	...
	//2
	else if(ctr->type==TimeConstraintType::CONSTRAINT_TWO_ACTIVITIES_CONSECUTIVE){
		ModifyConstraintTwoActivitiesConsecutiveForm form(this, (ConstraintTwoActivitiesConsecutive*)ctr);
		setParentAndOtherThings(&form, this);
		form.exec();
	}
	//3
	else if(ctr->type==TimeConstraintType::CONSTRAINT_TWO_ACTIVITIES_ORDERED){
		ModifyConstraintTwoActivitiesOrderedForm form(this, (ConstraintTwoActivitiesOrdered*)ctr);
		setParentAndOtherThings(&form, this);
		form.exec();
	}
	//4
	else if(ctr->type==TimeConstraintType::CONSTRAINT_ACTIVITY_PREFERRED_TIME_SLOTS){
		ModifyConstraintActivityPreferredTimeSlotsForm form(this, (ConstraintActivityPreferredTimeSlots*)ctr);
		setParentAndOtherThings(&form, this);
		form.exec();
	}
	//5
	else if(ctr->type==TimeConstraintType::CONSTRAINT_ACTIVITY_PREFERRED_STARTING_TIMES){
	...
	//10
	else if(ctr->type==TimeConstraintType::CONSTRAINT_ACTIVITIES_SAME_STARTING_TIME){
		ModifyConstraintActivitiesSameStartingTimeForm form(this, (ConstraintActivitiesSameStartingTime*)ctr);
		setParentAndOtherThings(&form, this);
		form.exec();
	}
	//11
	else if(ctr->type==TimeConstraintType::CONSTRAINT_ACTIVITIES_SAME_STARTING_HOUR){
		ModifyConstraintActivitiesSameStartingHourForm form(this, (ConstraintActivitiesSameStartingHour*)ctr);
		setParentAndOtherThings(&form, this);
		form.exec();
	}
	//12
	else if(ctr->type==TimeConstraintType::CONSTRAINT_ACTIVITIES_SAME_STARTING_DAY){
	...
	//29
	else if(ctr->type==TimeConstraintType::CONSTRAINT_STUDENTS_MAX_GAPS_PER_WEEK){
		ModifyConstraintStudentsMaxGapsPerWeekForm form(this, (ConstraintStudentsMaxGapsPerWeek*)ctr);
		setParentAndOtherThings(&form, this);
		form.exec();
	}
	//30
	else if(ctr->type==TimeConstraintType::CONSTRAINT_STUDENTS_EARLY_MAX_BEGINNINGS_AT_SECOND_HOUR){
	...
	//33
	else if(ctr->type==TimeConstraintType::CONSTRAINT_STUDENTS_MAX_HOURS_DAILY){
	...
	//35
	else if(ctr->type==TimeConstraintType::CONSTRAINT_STUDENTS_MAX_HOURS_CONTINUOUSLY){
	...
	//37
	else if(ctr->type==TimeConstraintType::CONSTRAINT_STUDENTS_MIN_HOURS_DAILY){
		ModifyConstraintStudentsMinHoursDailyForm form(this, (ConstraintStudentsMinHoursDaily*)ctr);
		setParentAndOtherThings(&form, this);
		form.exec();
	}
	//38
	else if(ctr->type==TimeConstraintType::CONSTRAINT_ACTIVITIES_NOT_OVERLAPPING){
	...
	//40
	else if(ctr->type==TimeConstraintType::CONSTRAINT_MIN_GAPS_BETWEEN_ACTIVITIES){
		ModifyConstraintMinGapsBetweenActivitiesForm form(this, (ConstraintMinGapsBetweenActivities*)ctr);
		setParentAndOtherThings(&form, this);
		form.exec();
	}
	//41
	else if(ctr->type==TimeConstraintType::CONSTRAINT_ACTIVITY_ENDS_STUDENTS_DAY){
		ModifyConstraintActivityEndsStudentsDayForm form(this, (ConstraintActivityEndsStudentsDay*)ctr);
		setParentAndOtherThings(&form, this);
		form.exec();
	}
	//42
	else if(ctr->type==TimeConstraintType::CONSTRAINT_TEACHER_INTERVAL_MAX_DAYS_PER_WEEK){
		ModifyConstraintTeacherIntervalMaxDaysPerWeekForm form(this, (ConstraintTeacherIntervalMaxDaysPerWeek*)ctr);
		setParentAndOtherThings(&form, this);
		form.exec();
	}
	//43
	else if(ctr->type==TimeConstraintType::CONSTRAINT_TEACHERS_INTERVAL_MAX_DAYS_PER_WEEK){
	...
	//45
	else if(ctr->type==TimeConstraintType::CONSTRAINT_STUDENTS_INTERVAL_MAX_DAYS_PER_WEEK){
	...
	//47
	else if(ctr->type==TimeConstraintType::CONSTRAINT_TWO_ACTIVITIES_GROUPED){
	...
	//49
	else if(ctr->type==TimeConstraintType::CONSTRAINT_TEACHER_ACTIVITY_TAG_MAX_HOURS_CONTINUOUSLY){
	...
	//52
	else if(ctr->type==TimeConstraintType::CONSTRAINT_TEACHERS_MAX_DAYS_PER_WEEK){
		ModifyConstraintTeachersMaxDaysPerWeekForm form(this, (ConstraintTeachersMaxDaysPerWeek*)ctr);
		setParentAndOtherThings(&form, this);
		form.exec();
	}
	//53
	else if(ctr->type==TimeConstraintType::CONSTRAINT_THREE_ACTIVITIES_GROUPED){
		ModifyConstraintThreeActivitiesGroupedForm form(this, (ConstraintThreeActivitiesGrouped*)ctr);
		setParentAndOtherThings(&form, this);
		form.exec();
	}
	//54
	else if(ctr->type==TimeConstraintType::CONSTRAINT_MAX_DAYS_BETWEEN_ACTIVITIES){
		ModifyConstraintMaxDaysBetweenActivitiesForm form(this, (ConstraintMaxDaysBetweenActivities*)ctr);
		setParentAndOtherThings(&form, this);
		form.exec();
	}
	//55
	else if(ctr->type==TimeConstraintType::CONSTRAINT_TEACHERS_MIN_DAYS_PER_WEEK){
		ModifyConstraintTeachersMinDaysPerWeekForm form(this, (ConstraintTeachersMinDaysPerWeek*)ctr);
		setParentAndOtherThings(&form, this);
		form.exec();
	}
	//56
	else if(ctr->type==TimeConstraintType::CONSTRAINT_TEACHER_MIN_DAYS_PER_WEEK){
	...
	//62
	else if(ctr->type==TimeConstraintType::CONSTRAINT_STUDENTS_MAX_GAPS_PER_DAY){
		ModifyConstraintStudentsMaxGapsPerDayForm form(this, (ConstraintStudentsMaxGapsPerDay*)ctr);
		setParentAndOtherThings(&form, this);
		form.exec();
	}
	//63
	else if(ctr->type==TimeConstraintType::CONSTRAINT_ACTIVITIES_OCCUPY_MAX_TIME_SLOTS_FROM_SELECTION){
	...
	//64
	else if(ctr->type==TimeConstraintType::CONSTRAINT_ACTIVITIES_MAX_SIMULTANEOUS_IN_SELECTED_TIME_SLOTS){
	...
	//66
	else if(ctr->type==TimeConstraintType::CONSTRAINT_STUDENTS_MAX_DAYS_PER_WEEK){
...
void AllTimeConstraintsForm::removeConstraint()
{
	...
	if(lres==0){
		...
		if(ctr->type==TimeConstraintType::CONSTRAINT_BASIC_COMPULSORY_TIME){ //additional confirmation for this one
